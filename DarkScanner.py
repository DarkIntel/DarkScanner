...
def get_onion_list():
        ...
        if os.path.exists("DarkScanner.list"):

                with open("DarkScanner.list","rb") as fd:

                        stored_onions = fd.read().splitlines()
        else:
                print("No DarkScanner.list detected. Get it here: DarkIntel.io!")
                sys.exit(0)
...
def store_onion(onion):
        ...
        print("[++] Storing %s in DarkScanner.list." % onion)

        with codecs.open("DarkScanner.list","ab",encoding="utf8") as fd:
                fd.write("%s\n" % onion)
...
def run_onionscan(onion):
        ...
        print("[*] DarkScanning %s" % onion)

        # fire up onionscan
        process = subprocess.Popen(["onionscan","--webport=0","--jsonReport","--simpleReport=false","--dbdir=./DarkScannerDB",onion],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
...
def handle_timeout(process,onion):
        ...
        try:
                process.kill()
                print("[!!!] Killed the DarkScanner process.")
        except:
                pass
        ...
        with Controller.from_port(port=9051) as torcontrol:
                ...
                torcontrol.authenticate("DarkScanner")
                ...
                print("[!!!] We need to go deeper. Switching TOR identities.")
...
def process_results(onion,json_response):
        ...
        if not os.path.exists("DarkScanner_results"):
                os.mkdir("DarkScanner_results")

        # write out the JSON results of the scan
        with open("%s/%s.json" % ("DarkScanner_results",onion), "wb") as fd:
...
while count < len(onions):
        ...
        if os.path.exists("DarkScanner_results/%s.json" % onion):
